test_that("tracing only evals originating from base packages", {
  f <- function(my_param=c("a", "b")) {
    assertthat::see_if(is.character("A"))
    match.arg(my_param)
  }

  calls <- do_trace_eval(f(), evals_to_trace="base")

  expect_equal(nrow(calls), 1)
  expect_equal(calls$caller_expression, "match.arg(my_param)")
})

test_that("tracing only evals originating from non-base packages", {
  f <- function(my_param=c("a", "b")) {
    assertthat::see_if(is.character("A"))
    match.arg(my_param)
  }
  ...
  expect_equal(unique(calls$caller_package), "assertthat")
})

test_that("tracing only global evals", {
  session <- function() {
    f <- function(my_param=c("a", "b")) {
      assertthat::see_if(is.character("A"))
      match.arg(my_param)
    ...
    evil::trace_code(
      ...
      evals_to_trace="global"
    )
  }

  r <- callr::r(session)
  calls <- r$tables$calls
  calls <- calls[order(calls$eval_call_id), ]

  expect_equal(calls$eval_call_expression, c("eval(1)", 'eval("a")'))
...
##     calls <- do_trace_eval(source(tf), evals_to_trace="global")
##     browser()
##     1
##   })

##   expect_equal(calls$eval_call_srcref, "::global::main::1")
## })
